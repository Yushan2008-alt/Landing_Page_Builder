...
export function HeroEditor({ content: c, onChange }: Props) {
  return (
    <div className="flex flex-col gap-3 pb-4">
      <PresetSelector type="hero" onSelect={(preset) => onChange(preset)} />
      <div className="flex flex-col gap-3 px-4">
        ...
        <div className="border-t border-border pt-3 space-y-2">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">Button URL</Label>
            <Input value={c.secondaryCtaUrl} onChange={(e) => onChange({ secondaryCtaUrl: e.target.value })} className="h-8 text-sm" />
          </div>
        </div>
